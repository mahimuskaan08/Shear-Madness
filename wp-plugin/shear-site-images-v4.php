<?php
/*
 * Plugin Name: Shear Site Images V4
 * Description: Public REST endpoint returning ACF image fields from post ID 2.
 * Version:     4.0.0
...
add_action( 'rest_api_init', 'shear_v4_register_route' );

function shear_v4_register_route() {
    register_rest_route( 'shear/v1', '/site-images', array(
        'methods'             => 'GET',
        'callback'            => 'shear_v4_handler',
        'permission_callback' => '__return_true',
    ) );
}

function shear_v4_resolve_url( $v ) {
...
function shear_v4_resolve_gallery( $items ) {
    if ( empty( $items ) || ! is_array( $items ) ) return array();
    $urls = array();
    foreach ( $items as $item ) {
        $url = shear_v4_resolve_url( $item );
        if ( $url ) $urls[] = $url;
    }
    return $urls;
}

function shear_v4_handler() {
    $id = 2;

    $img = function ( $name ) use ( $id ) {
        return shear_v4_resolve_url( get_field( $name, $id ) );
    };

    $gallery = function ( $name ) use ( $id ) {
        return shear_v4_resolve_gallery( get_field( $name, $id ) );
    };
